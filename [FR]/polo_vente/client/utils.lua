...
Citizen.CreateThread(function()
    while true do
        ...
        if distanceNPC <= 2.5 then
            sleepThread = 5
            ESX.ShowHelpNotification("Appuyer sur ~INPUT_CONTEXT~ pour ouvrir le menu d'activitée illégal!")
...
Citizen.CreateThread(function()
    while true do
        ...
        local ped = PlayerPedId()
    local pedCoords = GetEntityCoords(ped)
        local distanceNPC = GetDistanceBetweenCoords(pedCoords, npcCooords2.coords, true)
        if distanceNPC <= 2.5 then
            sleepThread = 5
            ESX.ShowHelpNotification("Appuyer sur ~INPUT_CONTEXT~ pour ouvrir le menu d'activitée illégal!")
            if IsControlJustPressed(0, 38) then
