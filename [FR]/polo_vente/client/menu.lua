...
NatveVehMenu = NativeUI.CreateMenu("Menu illégal", "~b~Activité illégal.")
...
function menuIllegal(menu)
    local RecupTraiter = NativeUI.CreateItem("Vendre son Héroïne", "")
    RecupTraiter:RightLabel("💉")
    menu:AddItem(RecupTraiter)

    local Opium = NativeUI.CreateItem("Vendre son Opium", "")
    Opium:RightLabel("💉")
    menu:AddItem(Opium)
    ...
    local Ecstasy = NativeUI.CreateItem("Vendre son Ecstasy", "")
    Ecstasy:RightLabel("💉")
    menu:AddItem(Ecstasy)

    local Amphetamines = NativeUI.CreateItem("Vendre son Amphetamines", "")
    Amphetamines:RightLabel("💉")
    menu:AddItem(Amphetamines)

    local LSD = NativeUI.CreateItem("Vendre son LSD", "")
    LSD:RightLabel("💉")
    menu:AddItem(LSD)
...
NatveVehMenu2 = NativeUI.CreateMenu("Menu illégal", "~b~Activité illégal.")
_menuPool2:Add(NatveVehMenu2)

function menuIllegal2(menu)
    local Coke = NativeUI.CreateItem("Vendre sa Coke", "")
    Coke:RightLabel("💉")
    menu:AddItem(Coke)

    local Marijuana = NativeUI.CreateItem("Vendre sa Marijuana", "")
    Marijuana:RightLabel("💉")
    menu:AddItem(Marijuana)

    local Meth = NativeUI.CreateItem("Vendre sa Meth", "")
    Meth:RightLabel("💉")
    menu:AddItem(Meth)

    local Crack = NativeUI.CreateItem("Vendre du Crack", "")
    Crack:RightLabel("💉")
    menu:AddItem(Crack)

    local Morphine = NativeUI.CreateItem("Vendre sa Morphine", "")
    Morphine:RightLabel("💉")
    menu:AddItem(Morphine)

    local Ketamine = NativeUI.CreateItem("Vendre sa Ketamine", "")
    Ketamine:RightLabel("💉")
