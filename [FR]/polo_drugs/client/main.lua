...
AddEventHandler('polo_drugs:hasEnteredMarker', function(zone)
	if zone == 'Weed' then
		...
		CurrentActionMsg  = '~g~Recolter de la Weed'
	...
	if zone == 'Weed9'  then
		...
		CurrentActionMsg  = '~b~Recolter de la Weed'
		CurrentActionData = {zone= zone}
	end

	if zone == 'Weed10'  then
		CurrentAction     = 'weed_traitement'
		CurrentActionMsg  = '~b~Traiter de la Weed'
		CurrentActionData = {zone= zone}
	end
	...
	if zone == 'Coke'  then
		...
		CurrentActionMsg  = '~p~Recolter de la Coke'
		CurrentActionData = {zone= zone}
	end
	...
	if zone == 'Coke11'  then
		...
		CurrentActionMsg  = '~w~Traiter de la Coke'
		CurrentActionData = {zone= zone}
	end
	...
	if zone == 'Ketamine'  then
		...
		CurrentActionMsg  = '~o~Recolter de la Ketamine'
		CurrentActionData = {zone= zone}
	end

	if zone == 'Ketamine2'  then
		CurrentAction     = 'ketamine_traitement'
		CurrentActionMsg  = '~b~Traiter de la Ketamine'
		CurrentActionData = {zone= zone}
	end

	if zone == 'LSD'  then
		CurrentAction     = 'LSD_recolte'
		CurrentActionMsg  = '~o~Recolter de la LSD'
		CurrentActionData = {zone= zone}
	end

	if zone == 'LSD2'  then
		CurrentAction     = 'LSD_traitement'
		CurrentActionMsg  = '~b~Traiter de la LSD'
		CurrentActionData = {zone= zone}
	end

	if zone == 'Morphine'  then
		CurrentAction     = 'morphine_recolte'
		CurrentActionMsg  = '~o~Recolter de la Morphine'
		CurrentActionData = {zone= zone}
	end
	...
	if zone == 'Morphine3'  then
		...
		CurrentActionMsg  = '~b~Traiter de la Morphine'
		CurrentActionData = {zone= zone}
	end

	if zone == 'Lean'  then
		CurrentAction     = 'lean_recolte'
		CurrentActionMsg  = '~o~Recolter de la Lean'
		CurrentActionData = {zone= zone}
	end

	if zone == 'Lean2'  then
		CurrentAction     = 'lean_traitement'
		CurrentActionMsg  = '~b~Traiter de la Lean'
		CurrentActionData = {zone= zone}
	end

	if zone == 'Heroine'  then
		CurrentAction     = 'heroine_recolte'
		CurrentActionMsg  = '~o~Recolter de la Heroine'
		CurrentActionData = {zone= zone}
	end

	if zone == 'Heroine2'  then
		CurrentAction     = 'heroine_traitement'
		CurrentActionMsg  = '~b~Traiter de la Heroine'
		CurrentActionData = {zone= zone}
	end
	...
	if zone == 'Marijuana'  then
		...
		CurrentActionMsg  = '~b~Recolter de la Marijuana'
		CurrentActionData = {zone= zone}
	end
	...
	if zone == 'Marijuana5'  then
		...
		CurrentActionMsg  = '~b~Traiter de la Marijuana'
		CurrentActionData = {zone= zone}
	end

	if zone == 'LaptopCoke'  then
		CurrentAction     = 'laptop_coke'
		CurrentActionMsg  = '~b~Allumez l\'ordinateur'
		CurrentActionData = {zone= zone}
	end
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and (GetGameTimer() - GUI.Time) > 300 then
				...
				-- Laptop en plus au cas ou vous n'avez pas le point, si vous l'avez supprimer celui là.
...
function Holograms()
		...
		if GetDistanceBetweenCoords( -708.94, 642.13, 155.17, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
			Draw3DText( -708.94, 642.13, 155.17  -1.400, "~g~Traiter de la Marijuana", 4, 0.1, 0.1)
		end
		...
		if GetDistanceBetweenCoords( -1797.97, 408.92, 113.66, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
			Draw3DText( -1797.97, 408.92, 113.66  -1.600, "~p~Traiter de la Ketamine", 4, 0.1, 0.1)
		end
		...
		if GetDistanceBetweenCoords( 244.67, 374.58, 105.73, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
			Draw3DText( 244.67, 374.58, 105.73  -1.600, "~y~Traiter de la LSD", 4, 0.1, 0.1)
		end
		...
		if GetDistanceBetweenCoords( 166.64, 2229.20, 90.76, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
			Draw3DText( 166.64, 2229.20, 90.76  -1.600, "~j~Traiter de la ~j~Morphine", 4, 0.1, 0.1)
		end
		...
		if GetDistanceBetweenCoords( 725.79, -1071.51, 28.31, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
			Draw3DText( 725.79, -1071.51, 28.31  -1.600, "~m~Traiter de la ~p~Lean", 4, 0.1, 0.1)
		end
...
function OpenPoloDrugsMenu(data)
		...
		ESX.UI.Menu.Open(
			'default', GetCurrentResourceName(), 'drugs',
			{
				...
				elements = {
				{label = 'Tenue de citoyen', value = 'citizen_wear'},
				{label = 'Tenue de drogues', value = 'drugs_wear'},			
			},
			},
			function(data, menu)
	...
	while true do
		...
		for i=1, #Config.Zoness, 1 do
			if(GetDistanceBetweenCoords(coords, Config.Zoness[i].x, Config.Zoness[i].y, Config.Zoness[i].z, true) < Config.ZoneSize.x) then
				...
				SetTextComponentFormat('STRING')
            	AddTextComponentString('appuyez sur ~INPUT_PICKUP~ pour prendre votre ~b~tenue~s~.')
            	DisplayHelpTextFromStringLabel(0, 0, 1, -1)
			end
		end
